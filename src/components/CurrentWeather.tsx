...
export default function CurrentWeather(props: WeatherProps) {
  ...
  useEffect(() => {
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?lat=${props.lat}&lon=${props.lon}&appid=${process.env.REACT_APP_API_KEY}&units=metric&lang=kr`
    )
      .then((res) => res.json())
      .then((data) => {
